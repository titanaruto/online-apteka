...
//    }
    ...
    private function request($url) {
        $ch = curl_init($url);
        curl_setopt($ch, CURLOPT_HTTPHEADER, array("Authorization: Bearer {$this->bearer}", "Content-Type: application/json"));
        curl_setopt($ch, CURLOPT_HEADER, 0);
...
//        return $response;
        ...
        return self::object2array($sxe);
    }

    function modelAreas() {
        return $this->request('https://ukrposhta.ua/address-classifier-ws/get_regions_by_region_ua');
    }

    function modelCities($id) {
        return $this->request("https://ukrposhta.ua/address-classifier-ws/get_city_by_region_id_and_district_id_and_city_ua?region_id={$id}");
    }

...
//namespace local\Sgus\Search\Levenshtein;


use Bitrix\Main\Entity\ExpressionField;
use Oefenweb\DamerauLevenshtein\DamerauLevenshtein;
//use \local\Sgus\Search\Levenshtein\ORM\DictionaryTable;
require_once( $_SERVER["DOCUMENT_ROOT"] . "/local/Sgus/Search/Levenshtein/ORM/DictionaryTable.php" );
...
class LevenshteinSearch
{
    ...
    {
        $result = [];
        $word_metaphone = LevenshteinDictionary::mtphn($word);
    ...
    public function findWord($word)
    {
        ...
        if (strlen($word) < 3 || !LevenshteinDictionary::filterWord($word, false)) {
            return [$word];
        }
        ...
        $word_convent = LevenshteinDictionary::conventLang($word, LevenshteinDictionary::getLang($word));

        if (!$word_metaphone && !$word_convent) {
            return [$word];
        }

        $word_convent_metaphone =  LevenshteinDictionary::mtphn($word_convent);
        ...
        $dbRes = DictionaryTable::getList([
            ...
            'runtime' => array(
                new ExpressionField('DAMLEVLIM', 'levenshtein(\''.$DB->ForSql($word_metaphone).'\', UF_METAPHONE)'),
                new ExpressionField('DAMLEVLIM_CONVENT', 'levenshtein(\''.$DB->ForSql($word_convent_metaphone).'\', UF_METAPHONE)'),
...
//            $item['PHP_DAMLEV_DISTANCE'] = $phpDamLevDistance;
        ...
        return self::array_pluck($result, 'UF_WORD');
    }

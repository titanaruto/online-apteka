...
//pre($_POST['ORDER_PROP_9']);
// обработка поля дисконтной карты
/*
if(!empty($_POST['ORDER_PROP_9'])) {
	global $USER;
	$rsUser = CUser::GetByID($USER->GetID());
	$arUser = $rsUser->Fetch();
	if(array_key_exists("UF_DK_USER", $arUser)){
		$_POST['ORDER_PROP_9'] = preg_replace("/[^0-9\(\)]/", '', $_POST['ORDER_PROP_9']);
		$newValue = $arUser['UF_DK_USER'];
		$newValue[] = $text;
		$user = new CUser;
		$fields = Array( 
			"UF_DK_USER" => $newValue
...
// оплачивать с аккаутна пользователя; 
...
// [Y|N] При отмеченной опции пользователю будет предложено оплатить с внутреннего счета, при условии, 
// что на этом счете есть достаточная сумма в базовой валюте сайта.
...
//колонка цена
//$arResult[GRID][ROWS][86][data][PRICE_FORMATED] = "102,00 грн.";

//колонка цены за строку заказа
//$arResult[GRID][ROWS][86][data][SUM] = "102,00 грн.";

//набор корзины заказа
//$arResult[BASKET_ITEMS];

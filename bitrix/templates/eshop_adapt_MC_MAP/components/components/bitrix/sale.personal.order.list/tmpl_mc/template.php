							...
							<tr>
								<td>
									...
									if($order['ORDER']['STATUS_ID']=='B' || $order['ORDER']['STATUS_ID']=='N') {
										echo '<span style="color:red;">Проверяется</span>';
									} else {
										echo $order["ORDER"]["FORMATED_PRICE"];
									}
									?> <br />
								...
								<td>
									...
									if($order['ORDER']['STATUS_ID'] != 'C') {
										...
										?>
										<div class="bx_my_order_status green" style="background: green !important;" onclick="lok(<?=$order['ORDER']['ID']?>)" >ОПЛАТИТЬ</div>
									<?
									}

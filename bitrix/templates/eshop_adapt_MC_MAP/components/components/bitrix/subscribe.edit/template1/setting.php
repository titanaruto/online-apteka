...
<thead class="www"><tr><td colspan="2"><?echo GetMessage("subscr_title_settings")?></td></tr></thead>
<tr valign="top">
	<td width="40%">
		...
		<input id="check_news" type="checkbox" name="RUB_ID[]" value="<?=$itemValue["ID"]?>"<?if($itemValue["CHECKED"]) echo ""?> />
			<label for="check_news"><?=$itemValue["NAME"]?></label><br />
		<?endforeach;?></p>
		<p><?echo GetMessage("subscr_fmt")?><br />
		<input id="radio_sub_one" type="radio" name="FORMAT" value="text"<?if($arResult["SUBSCRIPTION"]["FORMAT"] == "text") echo ""?> />
		<label for="radio_sub_one"><?echo GetMessage("subscr_text")?></label>
		<input id="radio_sub_two" type="radio" name="FORMAT" value="html"<?if($arResult["SUBSCRIPTION"]["FORMAT"] == "html") echo ""?> />
		<label for="radio_sub_two">HTML</label></p>
	</td>
	<td width="60%">

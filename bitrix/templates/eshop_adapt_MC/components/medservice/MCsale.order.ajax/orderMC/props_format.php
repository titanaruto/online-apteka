			...
			?>
				<div class="section-calc">
					...
					foreach ($arSource as $arProperties) {
                        ...
                        if ('Скрытые' == $arProperties["GROUP_NAME"]) {
                            $isHidden = true;
                        }

						$own_text = $arProperties["DESCRIPTION"];
						$own_text_result = str_replace($own_text, '', 'Заполните номер дисконтной карты для накопления на карте. Скидки по дисконтным картам на заказы НЕ ДЕЙСТВУЮТ!');

						//pre($own_text_result);
						?>
                        <div <?php if($isHidden) echo 'style="display: none;"';?> data-property-id-row="<?= intval(intval($arProperties["ID"])) ?>">
						...
						if ($arProperties["TYPE"] == "CHECKBOX") {
                            ...
                            <div class="bx_block r3x1">
                                <input type="tel" maxlength="250" size="<?=$arProperties["SIZE1"]?>" value="<?=$arProperties["VALUE"]?>" name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>" />
                                <?php
                                if (strlen(trim($arProperties["DESCRIPTION"])) > 0):
                                    ?>
                                    <div class="bx_description">
							...
							<div class="bx_block r3x1">
								...
								<!-- <div class="mc-button calc" onclick="checkDK()" ><i class="fa fa-calculator" aria-hidden="true"></i>Пересчитать</div> -->
								<!--button style="margin-top: 30px; clear: both; float: none;" class="><i class="fa fa-calculator" aria-hidden="true"></i>пересчитать</button-->
								<?}
								if (strlen(trim($arProperties["DESCRIPTION"])) > 0):
								?>
								<div class="bx_description">

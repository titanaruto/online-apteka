			...
			<div id="form_comment_" style="display:none;">
				...
				<div class="blog-comment-fields">
						...
						<div class="blog-comment-field blog-comment-field-user">
							<div class="blog-comment-field blog-comment-field-author">
								<div class="blog-comment-field-text">
									<label for="user_name"><?=GetMessage("B_B_MS_NAME")?>&nbsp;<span class="blog-required-field">*</span></label>
								</div>
								<span>
									<input maxlength="255" size="30" tabindex="3" type="text" name="user_name" id="user_name" value="<?=htmlspecialcharsEx($_SESSION["blog_user_name"])?>">
								</span>
							</div>
							...
							<div class="blog-comment-field blog-comment-field-email">
								...
								<span>
									<input maxlength="255" size="30" tabindex="4" type="text" name="user_email" id="user_email" value="<?=htmlspecialcharsEx($_SESSION["blog_user_email"])?>">
								</span>
							</div>
							<div class="blog-clear-float"></div>
							
							<label class="lastLabel" for="">Отзыв<span class="blog-required-field">*</span></label>
						</div>
						<?
					}
					?>
					...
					<div class="blog-comment-buttons">
                        <span name="sub-post" id="post-button" onclick="submitComment()">Отправить отзыв</span>
<!--                        <button onclick="submitComment()">Отправить отзыв</button>-->
<!--						<input class="Request_a_call" tabindex="10" value="Отправить отзыв--><?///*=GetMessage("B_B_MS_SEND")*/?><!--" type="button" name="sub-post" id="post-button" onclick="submitComment()">-->
					</div>
				</div>
...
//				if($comment["PUBLISH_STATUS"] == BLOG_PUBLISH_STATUS_PUBLISH || $comment["SHOW_SCREENNED"] == "Y" || $comment["ID"] == "preview")
				...
				elseif ($comment["PUBLISH_STATUS"] == 'M') {
				    if ($comment["AUTHOR_ID"] === CUser::GetID()) {
                        echo "<b>".GetMessage("COMMENT_ON_MODERATION")."</b>";
                    }
                }
				?>
				</div>
		...
		if($arResult["is_ajax_post"] != "Y")
		{
			if($arResult["CanUserComment"])
			{
				...
				<div class="blog-add-comment"><a class="bx_medium bx_bt_button" href="javascript:void(0)" onclick="return showComment('0')"><b>написать отзыв<?/*=GetMessage("B_B_MS_ADD_COMMENT")*/?></b></a><br /></div>
				<a name="0"></a>
				<?/*<span class="red_title_comments">написать отзыв</span>*/?>
				<?

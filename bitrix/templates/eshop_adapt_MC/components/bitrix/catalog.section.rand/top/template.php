...
foreach ($arResult['ITEMS'] as $key => $arItem){
    $product_price = (isset($arItem['RATIO_PRICE']) ? $arItem['RATIO_PRICE']["VALUE"] : $arItem['MIN_PRICE']["VALUE"]);
    $buy_status = Tools::isVisibleButtonForOrder($arItem["PHARM_LEFTOVERS"], $arItem["LAST_AVAILABILITY"], intval($product_price));
	$CPrice = new CPrice;
		$res = $CPrice->GetList(
			array(),
			array('PRODUCT_ID' => $arItem['ID'],'CATALOG_GROUP_ID' => intval(2))
		);
		if($arr = $res->Fetch()){
			$arItem['CATALOG_PRICE_2'] = $arr['PRICE'];
		}
	...
	?><div data-log="<?php print_r([$arItem["PHARM_LEFTOVERS"], $arItem["ID"], $arItem["XML_ID"], $arItem["LAST_AVAILABILITY"], $buy_status, $product_price]) ?>" class="<? echo ($arItem['SECOND_PICT'] ? 'bx_catalog_item double' : 'bx_catalog_item'); ?>">
    <div class="bx_catalog_item_container" id="<? echo $strMainID; ?>">
		<a id="<? echo $arItemIDs['PICT']; ?>"
           href="<? echo $arItem['DETAIL_PAGE_URL']; ?>"
           class="bx_catalog_item_images lazy"
           data-original="<?php echo $arItem['PREVIEW_PICTURE']['SRC']; ?>"
           title="<? echo $imgTitle; ?>"><?
	//расчитываем % скидки
	$arParams['SHOW_DISCOUNT_PERCENT'] = 'N';
	if($arItem['CATALOG_PRICE_2'] > $arItem['CATALOG_PRICE_1'] && $arItem['CATALOG_PRICE_1']!= 0){
		$a = $arItem['CATALOG_PRICE_2'];
		$b = $arItem['CATALOG_PRICE_1'];
		$c = round((1-($b/$a))*100);
		$minPrice['DISCOUNT_DIFF_PERCENT'] = $c;
		$arParams['SHOW_DISCOUNT_PERCENT'] = 'Y';
	}
	// отображаем % скидки
	if ('Y' == $arParams['SHOW_DISCOUNT_PERCENT']){
	?>
		<div id="<? echo $arItemIDs['DSC_PERC']; ?>" class="bx_stick_disc right bottom" <? echo (0 < $minPrice['DISCOUNT_DIFF_PERCENT'] ? '' : 'style="display:none;"'); ?>>-<? echo $minPrice['DISCOUNT_DIFF_PERCENT']; ?>%</div>
	<?
	}
	//if ($arItem['LABEL']){
	?>
			<div id="<? echo $arItemIDs['STICKER_ID']; ?>" class="bx_stick average left top" title="<? echo 'ТОП ПРОДАЖ'/*$arItem['LABEL_VALUE']*/; ?>"><? echo 'ТОП ПРОДАЖ'/*$arItem['LABEL_VALUE']*/; ?></div>
	<?
	//}
	?>
		</a><?
	if ($arItem['SECOND_PICT']){
		...
		if ('Y' == $arParams['SHOW_DISCOUNT_PERCENT']){
		?>
			<div id="<? echo $arItemIDs['SECOND_DSC_PERC']; ?>" class="bx_stick_disc right bottom" <? echo (0 < $minPrice['DISCOUNT_DIFF_PERCENT'] ? '' : 'style="display:none;"'); ?>>-<? echo $minPrice['DISCOUNT_DIFF_PERCENT']; ?>%</div>
		<?
		}
		//if ($arItem['LABEL']){
		?><div class="bx_stick average left top" title="<? echo 'ТОП ПРОДАЖ'/*$arItem['LABEL_VALUE']*/; ?>"><? echo 'ТОП ПРОДАЖ'/*$arItem['LABEL_VALUE']*/; ?></div>
	...
	<?if ('Y' != $arParams['SHOW_DISCOUNT_PERCENT']){?>
	<div class="sails-price">
		<span class="percent doverie"><?=round($arItem['CATALOG_PRICE_1']*0.95, 2).' грн';?></span>
		<span class="percent vip"><?=round($arItem['CATALOG_PRICE_1']*0.93, 2).' грн';?></span>
		<span class="percent zabota"><?=round($arItem['CATALOG_PRICE_1']*0.93, 2).' грн';?><?/*=round($arItem['CATALOG_PRICE_1']*0.9, 2).' грн';*/?></span>
	</div>
	<?}?>
            <?php if (!in_array($buy_status, [3, 4])) { ?>
                ...
                <div id="<? echo $arItemIDs['PRICE']; ?>" class="bx_price"><?
                    ...
                    if (!empty($minPrice) && $mPriceTrigerEgoMat != 0) {
                        if ('N' == $arParams['PRODUCT_DISPLAY_MODE'] && isset($arItem['OFFERS']) && !empty($arItem['OFFERS'])) {
                            ...
                            $ceil_price = explode(" ", $minPrice['PRINT_DISCOUNT_VALUE']);
                            echo ceil(str_replace(",", ".", $minPrice['PRINT_DISCOUNT_VALUE'])) . " " . $ceil_price[1];
                            $min_price = str_replace(",", ".", preg_replace('~[^0-9,]+~', '', $minPrice['PRINT_DISCOUNT_VALUE']));
                        }
                        ...
                        //!!!!
                        if ($arItem['CATALOG_PRICE_2'] > $arItem['CATALOG_PRICE_1']
                            && $arItem['CATALOG_PRICE_1'] != 0) {
                            echo '<span> ';
                    ...
                    unset($minPrice);
                    ?>
                    <meta itemprop="lowprice" content="<?= $min_price ?>">
	...
	if (!isset($arItem['OFFERS']) || empty($arItem['OFFERS']))
	{
		...
		<div id="<? echo $arItemIDs['BASKET_ACTIONS']; ?>" class="bx_catalog_item_controls_blocktwo">
            <?php if ($buy_status == 1) {?>
            <button id="<? echo $arItemIDs['BUY_LINK']; ?>" class="mc-button new-button-style" rel="nofollow"><?
            ...
            <?php } else if (in_array($buy_status, [2, 3])) {?>
                <script id="bx24_form_button" data-skip-moving="true">
                    ...
                    b24form({
                        ...
                        "click": "",
                        "presets": {
                            "my_param1": "<?=$arItem["NAME"];?>",
                            "my_param2": "<?=$arItem["XML_ID"];?>",
                            "my_param3": "<?= $arItem["PRICES"]["BASE"]["VALUE"] ?>"
                        }
                    });
                </script>
...
var <? echo $strObName; ?> = new JCCatalogSection(<? echo CUtil::PhpToJSObject($arJSParams, false, true); ?>);
	...
	else
	{
		...
		<div id="<? echo $arItemIDs['BASKET_ACTIONS']; ?>" class="bx_catalog_item_controls_blocktwo" style="display: <? echo ($canBuy ? '' : 'none'); ?>;">
            <?php
            if (!Tools::isVisibleButtonForOrder($arItem["PHARM_LEFTOVERS"], $arItem["LAST_AVAILABILITY"])) {?>
            <button id="<? echo $arItemIDs['BUY_LINK']; ?>" class="mc-button new-button-style" rel="nofollow"><?
			if ($arParams['ADD_TO_BASKET_ACTION'] == 'BUY')
			{
				echo ('' != $arParams['MESS_BTN_BUY'] ? $arParams['MESS_BTN_BUY'] : GetMessage('CT_BCS_TPL_MESS_BTN_BUY'));
            ...
            <?php } else {?>
                ...
                <button class="b24-web-form-popup-btn-8 mc-button <?= $mPriceTrigerEgoMat == 0 ? 'zakaz' : '' ?>"> Под заказ </button>
            <?php }?>
		</div>
		<?
	if ($arParams['DISPLAY_COMPARE'])

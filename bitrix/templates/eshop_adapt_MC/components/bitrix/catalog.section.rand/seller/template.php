...
foreach ($arResult['ITEMS'] as $key => $arItem){
	...
	?>
	<div data-log="<?php print_r([$arItem["PHARM_LEFTOVERS"], $arItem["ID"], $arItem["XML_ID"], $arItem["LAST_AVAILABILITY"], $buy_status, $product_price])?>" class="<? echo ($arItem['SECOND_PICT'] ? 'bx_catalog_item double' : 'bx_catalog_item'); ?>" itemprop='description'>
		<div class="bx_catalog_item_container" id="<? echo $strMainID; ?>">
		<a id="<? echo $arItemIDs['PICT']; ?>" href="<? echo $arItem['DETAIL_PAGE_URL']; ?>" class="bx_catalog_item_images lazy"
	...
	//if ($arItem['LABEL']){
	/*?>
			<div id="<? echo $arItemIDs['STICKER_ID']; ?>" class="bx_stick average left top" title="<? echo 'АКЦИОННОЕ ПРЕДЛОЖЕНИЕ'/*$arItem['LABEL_VALUE']*/; /*?>"><? echo 'АКЦИОННОЕ ПРЕДЛОЖЕНИЕ'/*$arItem['LABEL_VALUE']*//*; ?></div>
	<?*/
	//}
	?>
		</a><?
	...
	?>
            <div>
                <div class="bx_catalog_item_title" itemprop="name"><a href="<? echo $arItem['DETAIL_PAGE_URL']; ?>" title="<? echo $productTitle; ?>"><? echo $productTitle; ?></a></div>
            </div>
	<?if ('Y' != $arParams['SHOW_DISCOUNT_PERCENT']){?>
	<div class="sails-price">
		...
		<span class="percent zabota"><?=round($arItem['CATALOG_PRICE_1']*0.9, 2).' грн';?></span>
	</div>
	<?}?>
                ...
                <div id="<? echo $arItemIDs['PRICE']; ?>" class="bx_price"><?
                    ...
                    if (!empty($minPrice)) {
                        ...
                        if ($arItem['CATALOG_PRICE_2'] > $arItem['CATALOG_PRICE_1']
                            ...
                            echo '<span class="catalog-price-2-span"> ';

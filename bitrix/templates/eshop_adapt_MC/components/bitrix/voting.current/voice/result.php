...
?>
<span id="statistic" class="mc-button result-sts">Посмотреть / Скрыть статистику</span>
<div id="<?=$id?>_result" class="own-result">
	...
	<?if ($arParams["SHOW_RESULTS"] = "Y" && $arParams["CAN_VOTE"] == "Y"):?>
		<div class="vote-form-box-buttons vote-vote-footer vote-result mc-button">
...
<!-- View statistic vote -->
<script>
	  var menuElem = document.getElementById('statistic');
	  var find = document.querySelector('.own-result');
	  find.classList.add('result-open');

	  menuElem.onclick = function() {
	    find.classList.toggle('result-open');
	  };
</script>

	...
	<div class="bx-filter-section container-fluid">
		...
		<form name="<?echo $arResult["FILTER_NAME"]."_form"?>" action="<?echo $arResult["FORM_ACTION"]?>" method="get" class="smartfilter">
			...
			<div class="row">
					...
					if(isset($arItem["PRICE"])):
						...
						<div class="<?if ($arParams["FILTER_VIEW_MODE"] == "HORIZONTAL"):?>col-sm-6 col-md-4<?else:?>col-lg-12<?endif?> bx-filter-parameters-box bx-active">
							...
							<div class="bx-filter-block" data-role="bx_filter_block">
                                ...
                                <?
                                //max
                                if (!empty($arItem["VALUES"]["MAX"]["HTML_VALUE"]) || $arItem["VALUES"]["MAX"]["HTML_VALUE"] != ""){
                                    $maxFilter['value'] = $arItem["VALUES"]["MAX"]["HTML_VALUE"];
                                } else if(empty($arItem["VALUES"]["MAX"]["HTML_VALUE"]) || $arItem["VALUES"]["MAX"]["HTML_VALUE"] == ""){
                                    $maxFilter['value'] = $arItem["VALUES"]["MAX"]["VALUE"];
                                }

                                $maxFilter['id'] = $arItem["VALUES"]["MAX"]["CONTROL_ID"];
                                $maxFilter['name'] = $arItem["VALUES"]["MAX"]["CONTROL_NAME"];

                                //min
                                if (!empty($arItem["VALUES"]["MIN"]["HTML_VALUE"]) || $arItem["VALUES"]["MIN"]["HTML_VALUE"] != ""){
                                    $minFilter['value'] = $arItem["VALUES"]["MIN"]["HTML_VALUE"];
                                } else if(empty($arItem["VALUES"]["MIN"]["HTML_VALUE"]) || $arItem["VALUES"]["MIN"]["HTML_VALUE"] == ""){
                                    $minFilter['value'] = $arItem["VALUES"]["MIN"]["VALUE"];
                                }
                                $minFilter['id'] = $arItem["VALUES"]["MIN"]["CONTROL_ID"];
                                $minFilter['name'] = $arItem["VALUES"]["MIN"]["CONTROL_NAME"];
                                ?>
				...
				foreach($arResult["ITEMS"] as $key=>$arItem)
				{
					...
					<div class="<?if ($arParams["FILTER_VIEW_MODE"] == "HORIZONTAL"):?>col-sm-6 col-md-4<?else:?>col-lg-12<?endif?> bx-filter-parameters-box <?if ($arItem["DISPLAY_EXPANDED"]== "Y"):?>bx-active<?endif?>">
						...
						<div class="bx-filter-parameters-box-title" onclick="smartFilter.hideFilterProps(this)">
							<span <?php if ($arItem["CODE"] == "BRAND" && count($arItem["VALUES"]) == 1) echo "style='display: none'"?> class="bx-filter-parameters-box-hint"><?=$arItem["NAME"]?>
								...
								default://CHECKBOXES
									...
									<?foreach($arItem["VALUES"] as $val => $ar):?>
										<div class="checkbox_div">
											<label data-role="label_<?=$ar["CONTROL_ID"]?>" class="qwe bx-filter-param-label <? echo $ar["DISABLED"] ? 'disabled': '' ?>" for="<? echo $ar["CONTROL_ID"] ?>">
												<span class="bx-filter-input-checkbox">
													<input class="checkboxes" type="checkbox" value="<? echo $ar["HTML_VALUE"] ?>" name="<? echo $ar["CONTROL_NAME"] ?>" id="<? echo $ar["CONTROL_ID"] ?>" <? echo $ar["CHECKED"]? 'checked="checked"': '' ?> onclick="smartFilter.click(this)"/>
													<label class="check" for="<? echo $ar["CONTROL_ID"] ?>"></label>
													<span class="bx-filter-param-text" title="<?=$ar["VALUE"];?>"><?=$ar["VALUE"];?><?
													if ($arParams["DISPLAY_ELEMENT_COUNT"] !== "N" && isset($ar["ELEMENT_COUNT"])):
														?>&nbsp;(<span data-role="count_<?=$ar["CONTROL_ID"]?>"><? echo $ar["ELEMENT_COUNT"]; ?></span>)<?
													endif;?></span>
				...
				?>
                <div class="sort-smart-filter">
                    <div>Сортировать по цене:</div>
                    <a href="?sort=price&order=lover">по убыв.</a> | <a href="?sort=price&order=upper">по возвр.</a>
                    <div>Сортировать по</div> <a href="?sort=name">названию</a>
                </div>
			...
			<div class="row">
				<div class="col-xs-12 bx-filter-button-box">
					<div class="bx-filter-block">
						<div class="bx-filter-parameters-box-container">
							<button style="float: none; margin-top: 10px;" class="mc-button search" type="submit" id="set_filter" name="set_filter"><i class="fa fa-search"></i><?=GetMessage("CT_BCSF_SET_FILTER")?></button>
							<button class="mc-button ban" id="del_filter333" name="del_filter" value="" ><i class="fa fa-ban"></i><?=GetMessage("CT_BCSF_DEL_FILTER")?></button>
...
<script>
    $( function() {
        let min = <?=$minFilter['value']?>;
        let max = <?=$maxFilter['value']?>;
        let minObj = document.getElementById("<?=$minFilter['id']?>");
        $( "#slider-range" ).slider({
            range: true,
            ...
            slide: function( event, ui ) {
                ...
                $( "#<?=$minFilter['id']?>" ).val( ui.values[ 0 ]  );
                $( "#<?=$maxFilter['id']?>" ).val( ui.values[ 1 ] );
                smartFilter.click(minObj);
                // onclick="smartFilter.click(this)"

            }
        });

        let filterInputMinPrice = $(".bx-filter-parameters-box-container input.min-price").val()
        let filterInputMaxPrice = $(".bx-filter-parameters-box-container input.max-price").val()
        let checkedFilters = $("form.smartfilter input:checked").length
        ...
        console.log(filterInputMinPrice, checkedFilters, findGetParameter('sort'), filterInputMaxPrice, <?=$arResult["ITEMS"]["BASE"]["VALUES"]["MAX"]["VALUE"]?>)

        if (filterInputMinPrice < <?=$arResult["ITEMS"]["BASE"]["VALUES"]["MIN"]["VALUE"]?> || checkedFilters > 0 || findGetParameter('sort') || filterInputMaxPrice < <?=$arResult["ITEMS"]["BASE"]["VALUES"]["MAX"]["VALUE"]?>) {
            $(".bx-filter-button-box #del_filter333").show()
        }


        $( "#slider-range" ).slider({
            stop: function( event, ui ) {
                $(".bx-filter-button-box #del_filter333, .bx-filter-button-box #set_filter").fadeIn()
            }
        });
        ...
        $("#del_filter333").on('click', function () {
            let urlArray = window.location.href.split('/')
            urlArray.splice(urlArray.indexOf('filter'))

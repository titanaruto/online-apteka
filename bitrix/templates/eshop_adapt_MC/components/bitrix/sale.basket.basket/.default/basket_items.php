...
<div id="basket_items_list">
	<div class="bx_ordercart_order_table_container">
		<table id="basket_items">
				<tr class="basket_items_head_row">
					<td class="margin">
<!--                        <input type="checkbox" id="select-all-products-in-cart">-->
                    </td>
					<?
...
$parent = '';
				foreach ($arResult["GRID"]["ROWS"] as $k => $arItem):
                    ...
                    $product_set = Tools::getProductSet($arItem["PRODUCT_ID"]);
//				pre($product_set);

				if (count($product_set) > 0) {
				    $parent = $arItem["PRODUCT_ID"];
                    $array = $product_set;
                }
...
//pre($array);
				...
				?>
					<tr class="row-goods" id="<?=$arItem["ID"]?>">
                        <td style="vertical-align: middle" class="margin">
                            <?php if (count($product_set) > 0) {?>
                                <div class="set-products-delete">
                                    <div class="arrow-bottom"></div>
                                    <?=GetMessage("SET_PRODUCTS_DELETE")?>
                                    <button class="mc-button basket-mc-button total-cart-delete-items set-products-delete-btn"><?echo GetMessage('TOTAL_CART_DELETE')?></button>
                                </div>
                            <?php }?>
                            <input <?=count($product_set) > 0 ? 'data-set-parent="' . $arItem["PRODUCT_ID"] . '"' : (in_array($arItem["PRODUCT_ID"], $array) ? 'data-set-chld=' . $parent : '')?> class="chkb-item-cart" type="checkbox" data-id="<?=$arItem["ID"]?>" id="chkb_<?=$arItem["ID"]?>"></td>
						<?
						foreach ($arResult["GRID"]["HEADERS"] as $id => $arHeader):
...
<!--									<span>--><?//=$arHeader["name"]; ?><!--:</span>-->
													...
													<div id="basket_quantity_control">
														<div class="basket_quantity_control">
                                                            <table border="0" cellspacing="0" cellpadding="0">
                                                                <tr>
                                                                    <td><a href="javascript:void(0);" class="minus" onclick="setQuantity(<?=$arItem["ID"]?>, <?=$arItem["MEASURE_RATIO"]?>, 'down', <?=$useFloatQuantityJS?>);"></a></td>
                                                                    <td><?
                                                                        $ratio = isset($arItem["MEASURE_RATIO"]) ? $arItem["MEASURE_RATIO"] : 0;
                                                                        $max = isset($arItem["AVAILABLE_QUANTITY"]) ? "max=\"".$arItem["AVAILABLE_QUANTITY"]."\"" : "";
                                                                        $useFloatQuantity = ($arParams["QUANTITY_FLOAT"] == "Y") ? true : false;
                                                                        ...
                                                                        <input
                                                                                ...
                                                                                onchange="updateQuantity('QUANTITY_INPUT_<?=$arItem["ID"]?>', '<?=$arItem["ID"]?>', <?=$ratio?>, <?=$useFloatQuantityJS?>)"
                                                                        ></td>
                                                                    <td valign="middle"><a href="javascript:void(0);" class="plus" onclick="setQuantity(<?=$arItem["ID"]?>, <?=$arItem["MEASURE_RATIO"]?>, 'up', <?=$useFloatQuantityJS?>);"></a></td>
                                                                </tr>
                                                            </table>
														</div>
													</div>
							...
							?>
								<td class="custom custom-persent">
<!--									<span>--><?//=$arHeader["name"]; ?><!--:</span>-->
                                    <?php if ($arItem["DISCOUNT_PRICE_PERCENT_FORMATED"] != "0%" && $arItem["DISCOUNT_PRICE_PERCENT_FORMATED"] != "-INF%") :?>
									    <div class="discount-value" id="discount_value_<?=$arItem["ID"]?>"><?php $arr_disc = ["-INF%", "NAN%"]; echo (in_array($arItem["DISCOUNT_PRICE_PERCENT_FORMATED"], $arr_disc) ? $arItem["DISCOUNT_PRICE_PERCENT_FORMATED"] : $arItem["DISCOUNT_PRICE_PERCENT_FORMATED"])?></div>
                                    <?php endif;?>
								</td>
							<?
							elseif ($arHeader["id"] == "WEIGHT"):
							...
							?>
								<td class="custom custom-sum">
									<p><?/*=$arHeader["name"]; */?>Итого</p>
									<?
									if ($arHeader["id"] == "SUM"):
									?>
						...
						?>
							<td class="control">
								<?
								if ($bDeleteColumn):
                                    $short_del_url = str_replace("#ID#", $arItem["ID"], $arUrls["delete"]);
								?>
<!--									<a class="mc-button delete main" href="--><?//=str_replace("#ID#", $arItem["ID"], $arUrls["delete"])?><!--">--><?///*=GetMessage("SALE_DELETE")*/?><!--</a><br />-->
									<a class="mc-button delete main" onclick="deleteProductFromCartById(<?=$arItem["ID"]?>)" href="javascript:void(0)"><?/*=GetMessage("SALE_DELETE")*/?></a><br />
								<?
								endif;
								if ($bDelayColumn):
								?>
									<a class="mc-button delete" href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["delay"])?>"><?/*=GetMessage("SALE_DELAY")*/?>x</a>
								<?
								endif;
								?>
							</td>
        ...
        <button class="mc-button basket-mc-button total-cart-delete-items"><?echo GetMessage('TOTAL_CART_DELETE')?></button>
	</div>
...
?>
		...
		<div class="bx_ordercart_order_pay_center">
			...
			*/?>
<!--			<a href="javascript:void(0)" onclick="checkOut();" style="line-height: 25px;" class="mc-button basket-mc-button">--><?//=GetMessage("SALE_ORDER")?><!--</a>-->
            <a href="/personal/order/make/" style="line-height: 25px;" class="mc-button basket-mc-button"><?=GetMessage("SALE_ORDER")?></a>
		</div>
	</div>
</div>
<?

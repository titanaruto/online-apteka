...
//pre($arResult['SECTION']['DEPTH_LEVEL']);
	...
	if(!isset($_GET['all']) && $arResult['SECTION']['DEPTH_LEVEL'] > 0 && $arResult['SECTION']['DEPTH_LEVEL'] < 3){
			echo '<a class="back-top" href="'.$_SERVER['REDIRECT_URL'].'/?all=Y">Показать все статьи</a>';
	}
	if(isset($_GET['all']) && $arResult['SECTION']['DEPTH_LEVEL'] > 0 && $arResult['SECTION']['DEPTH_LEVEL'] < 3){
	...
	if((!isset($_GET['all']) && $arResult['SECTION']['DEPTH_LEVEL'] > 0 && $arResult['SECTION']['DEPTH_LEVEL'] < 3 )
		...
		?><ul id="444" class="<? echo $arCurView['LIST']; ?>"><?
			switch ($arParams['VIEW_MODE'])
			{
				case 'LINE':
					foreach ($arResult['SECTIONS'] as &$arSection)
					{
						...
						?><li id="<? echo $this->GetEditAreaId($arSection['ID']); ?>" class="article-lists-pict">
				...
				case 'TEXT':
					foreach ($arResult['SECTIONS'] as &$arSection)
					{
						...
						?><li id="<? echo $this->GetEditAreaId($arSection['ID']); ?>" class="article-lists-pict"><h2 class="bx_catalog_text_title"><a href="<? echo $arSection['SECTION_PAGE_URL']; ?>"><? echo $arSection['NAME']; ?></a><?
						if ($arParams["COUNT_ELEMENTS"])
						{
							?> <span>(<? echo $arSection['ELEMENT_CNT']; ?>)</span><?
						}
				...
				case 'LIST':
					...
					foreach ($arResult['SECTIONS'] as &$arSection)
					{
						...
						?><li id="<?=$this->GetEditAreaId($arSection['ID']);?>" class="article-lists-pict"><h2 class="bx_sitemap_li_title"><a href="<? echo $arSection["SECTION_PAGE_URL"]; ?>"><? echo $arSection["NAME"];?><?
						if ($arParams["COUNT_ELEMENTS"])
						{
							?> <span>(<? echo $arSection["ELEMENT_CNT"]; ?>)</span><?
						}

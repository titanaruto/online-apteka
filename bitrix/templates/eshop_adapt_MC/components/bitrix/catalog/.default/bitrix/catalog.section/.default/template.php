...
$arFilter = Array('IBLOCK_ID'=>"2", 'ACTIVE' => 'Y','=CODE'=> $arParams['SECTION_CODE']);
$db_list = CIBlockSection::GetList(false, $arFilter, false);
$section_id = $db_list->Fetch()["ID"];

    $intCount = CIBlockSection::GetCount(array('IBLOCK_ID' => 2,'ACTIVE' => 'Y','SECTION_ID' => $section_id));

    if ($intCount == 0 ){
        $cols = 3;
    } else{
        $cols = $arParams['LINE_ELEMENT_COUNT'];
    }
	?>
	<div class="bx_catalog_list_home col<? echo $cols; ?> <? echo $templateData['TEMPLATE_CLASS']; ?>">
	<?
	foreach ($arResult['ITEMS'] as $key => $arItem) {
	    ...
	    if ($arItem['PRICES']['BASE']['VALUE'] == 0 && in_array($_GET["order"], ["lover", "upper"])) continue;
		//pre($arItem);
		$CPrice = new CPrice;
		$res = $CPrice->GetList(
			array(),
			array('PRODUCT_ID' => $arItem['ID'],'CATALOG_GROUP_ID' => intval(1))
		);
		if($arr = $res->Fetch()){
			//pre($arr);
			$arItem['CATALOG_PRICE_1'] = $arr['PRICE'];
		}
		$res = $CPrice->GetList(
			array(),
	...
	?><div data-log="<?php print_r([$arItem["PHARM_LEFTOVERS"], $arItem["ID"], $arItem["XML_ID"], $arItem["LAST_AVAILABILITY"], $buy_status, $product_price]) ?>" class="<?
		echo ($arItem['PICT'] ? 'bx_catalog_item double' : 'bx_catalog_item'); ?>"><div
		class="bx_catalog_item_container" id="<? echo $strMainID; ?>">
		<a id="<? echo $arItemIDs['PICT']; ?>"
			href="<? echo $arItem['DETAIL_PAGE_URL']; ?>"
			class="bx_catalog_item_images"
			title="<? echo $imgTitle; ?>">
            <img class="bx_catalog_item_section_img lazy" data-original="<? echo $arItem['PREVIEW_PICTURE']['SRC']; ?>" alt="<? echo $imgTitle; ?>">
            <?
	//расчитываем % скидки
	...
	if ($arItem['LABEL']){
	?><div id="<? echo $arItemIDs['STICKER_ID']; ?>"
		class="bx_stick average left top"
		title="<? echo $arItem['LABEL_VALUE']; ?>"><?
		echo $arItem['LABEL_VALUE']; ?></div><?
	}
	echo '</a>';
	...
	<?if ('Y' != $arParams['SHOW_DISCOUNT_PERCENT'] && true===empty($arItem['PROPERTIES']['CONSTANT_PRICE']['VALUE'])
	...
	<div class="sails-price">
		...
		<span class="percent zabota"><?=round($arItem['CATALOG_PRICE_1']*0.93, 2).' грн';?></span>
	</div>
	<?}?>
	...
	if (!isset($arItem['OFFERS']) || empty($arItem['OFFERS']))
	{
		...
		<div id="<? echo $arItemIDs['BASKET_ACTIONS']; ?>" class="bx_catalog_item_controls_blocktwo">
            ...
            <?php
            if ($buy_status == 1) {?>
				<button id="<? echo $arItemIDs['BUY_LINK']; ?>" class="mc-button" href="javascript:void(0)" rel="nofollow"><?
		...
		else
		{
			...
			echo ('' != $arParams['MESS_NOT_AVAILABLE'] ? 'Под заказ'/*$arParams['MESS_NOT_AVAILABLE']*/ : GetMessage('CT_BCS_TPL_MESS_PRODUCT_NOT_AVAILABLE'));
			?></span></div><?
...
<div class="<? echo $templateData['TEMPLATE_CLASS']; ?>">

...
<?$a = $_SERVER['REQUEST_URI'];
	...
	if (isset($_SERVER['REQUEST_URI']) && $zzzz['0']) {?>
		<?/*<h1 style="display: none;" id="basicTitle" class=""></h1>*/?>
		<style>
			#basicTitle{
				/*display: none;*/
			}
...
<?if ($curPage != SITE_DIR."index.php"):?>
	<h1	id="<? echo $this->GetEditAreaId($arResult['SECTION']['ID']); ?>"><?echo (isset($arResult['SECTION']["IPROPERTY_VALUES"]["SECTION_PAGE_TITLE"]) && $arResult['SECTION']["IPROPERTY_VALUES"]["SECTION_PAGE_TITLE"] != "" ? $arResult['SECTION']["IPROPERTY_VALUES"]["SECTION_PAGE_TITLE"] : $arResult['SECTION']['NAME']);?></h1>
...
/*
	if(!isset($_GET['all']) && $arResult['SECTION']['DEPTH_LEVEL'] > 0 && $arResult['SECTION']['DEPTH_LEVEL'] < 3){
			echo '<a class="back-top" href="'.$_SERVER['REDIRECT_URL'].'/?all=Y&clear_cache=Y">Показать все товары</a>';
	}
	if(isset($_GET['all']) && $arResult['SECTION']['DEPTH_LEVEL'] > 0 && $arResult['SECTION']['DEPTH_LEVEL'] < 3){
			echo '<h2>Все товары в категории '.$arResult['SECTION']['NAME'].'</h2>';
			echo '<a class="back-top" href="'.$_SERVER['REDIRECT_URL'].'?clear_cache=Y">Вернуться в каталог</a>';
	}
	if((!isset($_GET['all']) && $arResult['SECTION']['DEPTH_LEVEL'] > 0 && $arResult['SECTION']['DEPTH_LEVEL'] < 3 )
		||
			...
			switch ($arParams['VIEW_MODE'])
			{
				case 'LINE':
					foreach ($arResult['SECTIONS'] as &$arSection)
					{
						...
						?><li itemtype="http://schema.org/ItemList" id="<? echo $this->GetEditAreaId($arSection['ID']); ?>">
					...
					foreach ($arResult['SECTIONS'] as &$arSection)
					{
						...
						?><li itemtype="http://schema.org/ItemList" id="<? echo $this->GetEditAreaId($arSection['ID']); ?>">
							...
							?>
								...
								>
                        <img class="bx_catalog_item_section_img lazy" data-original="<? echo $arSection['PICTURE']['SRC']; ?>" alt="<? echo $arSection['PICTURE']['TITLE']; ?>">
							...
							{
								?><a class="bx_catalog_tile_title" href="<? echo $arSection['SECTION_PAGE_URL']; ?>"><? echo $arSection['NAME']; ?></a><?
								if ($arParams["COUNT_ELEMENTS"])
								{
									?> <span>(<? echo $arSection['ELEMENT_CNT']; ?>)</span><?
								}

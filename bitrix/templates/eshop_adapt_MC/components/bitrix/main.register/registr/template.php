...
?>
<div class="row">
<div class="bx-auth-reg col-md-8 col-lg-8">
...
<?else:?>
<?
$APPLICATION->SetAdditionalCSS("/bitrix/templates/eshop_adapt_MC/components/bitrix/socserv.auth.form/flat/style.css");
...
<script>
    function handle_fb_data(response){
        FB.api('/me?fields=id,email,link,last_name,first_name', function(response) {
            $.ajax({
                type: 'post',
                url: '/bitrix/templates/eshop_adapt_MC/components/bitrix/socserv.auth.form/flat/ajax/reg_user_fb.php',
                data:response,
...
?>
	...
	<tfoot>
		<tr>
			<td></td>
			<td><input type="submit" class="mc-button" name="register_submit_button" value="<?=GetMessage("AUTH_REGISTER")?>" /></td>
		</tr>
	</tfoot>
</table>
...
<?endif?>
    ...
    <div class="bx-authform-social col-md-4 col-lg-4">
        <p>Регистрация с помощью учетных записей социальных сетей:</p>
        <ul class="bx-authform-social-list">
            <li>
                <a title="Регистрация через Facebook" onclick="fb_login();"href="javascript:void(0)" class="btn btn-soc-facebook">Facebook</a>
            </li>
        </ul>
    </div>
</div>

...
// цена заказа
$arResult['PRICE']; //для расчета
$arResult['PRICE_FORMATED'];// для отображения
$arResult['PRODUCT_SUM_FORMATTED'];// для отображения
//цена товара
$arResult['BASKET']['93']['PRICE_FORMATED'];//для отображения
$arResult['BASKET']['93']['PRICE'];//для расчета
$arResult['BASKET']['93']['QUANTITY'];//количество по позиции для расчета
//pre($arResult['ORDER_PROPS']);
foreach($arResult['ORDER_PROPS'] as $propz) {
    if($propz['CODE'] == 'DKCARD') {
        $DKnumb = $propz['VALUE'];
    }
}
...
//pre($value);
unset($qwery);
//|| $order['ORDER']['STATUS_ID']=='A'
/*if($arResult['STATUS_ID']=='B' || $arResult['STATUS_ID']=='N' ) {
    if($value > 0) {
        $CPrice = new CPrice;
        $allOrdPr = '';
        foreach($arResult['BASKET'] as $key=>$row){
            ...
            $arr = $res->Fetch();
            if(empty($arr['PRICE']) || $arr['PRICE']== 0){
                //$arResult['BASKET'][$key]['PRICE'] = $row['PRICE'] - $row['PRICE']*$value;
                $price = round($row['PRICE'] - $row['PRICE']*$value,2);
                $arResult['BASKET'][$key]['PRICE'] = $price;
                $arResult['BASKET'][$key]['PRICE_FORMATED'] = $price.' грн.';
                $allOrdPr = $allOrdPr + $price*$row['QUANTITY'];
            } else {
                $allOrdPr = $allOrdPr + $row['PRICE']*$row['QUANTITY'];
            }
        }
        $allOrdPr = round($allOrdPr,2);
        $arResult['PRODUCT_SUM_FORMATTED'] = $allOrdPr.' грн.';
        $arResult['PRICE_FORMATED'] = $allOrdPr.' грн.';
    }
}*/
    ...
    <?else:?>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <?elseif($arResult["CAN_CANCEL"] == "Y"):?>
                                <?=GetMessage('SPOD_NO')?> [ <a href="<?=$arResult["URL_TO_CANCEL"]?>"
                                style="width:100px;display:inline;"><?=GetMessage("SPOD_ORDER_CANCEL")?></a> ]
                            <?endif?>
                        </td>
                    </tr>
                <?endif?>
        ...
        <table class="bx_order_list_table_order">
            <thead>
                <tr>
                    ...
                    <td class="custom price">Всего</td>
                </tr>
            </thead>
            <tbody>
                <?foreach($arResult["BASKET"] as $prod):?>
                    <tr>
                        ...
                        <td class="custom price">
                            <?
                            //pre($prod);
                            $produktRowTotalPrice=$prod["PRICE"]*$prod["QUANTITY"];
                            $produktRowTotalPrice = round($produktRowTotalPrice,2);?>
                            <span class="fm"><?=GetMessage('SPOD_QUANTITY')?>:</span> <?=$produktRowTotalPrice.' грн.'?>
                            <?if(strlen($prod['MEASURE_TEXT'])):?>
                                <?=$prod['MEASURE_TEXT']?>
                            <?else:?>
                                <?=GetMessage('SPOD_DEFAULT_MEASURE')?>
        ...
        <table class="bx_control_table" style="width: 100%;">
            <tr>
                ...
                <?if($arResult['STATUS_ID'] == 'C' ){?>
                    <td align="right"><a href=<?echo '"/personal/order/payment/?ORDER_ID='.$arResult['ID'].'"';?> class="bx_big bx_bt_button_type_2 bx_cart">Оплатить</a></td>
                <?}?>
            </tr>
        </table>
